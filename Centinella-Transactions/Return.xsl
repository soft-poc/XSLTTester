<?xml version="1.0" encoding="UTF-8"?>
    ...
    <xsl:variable name="TotalCreditAmount" select="sum($InvoiceNodeSet/paymentsummary/records[Type='Visa']/CapturedAmount)"/>
     <xsl:variable name="TotalCreditAmount2" select="sum($InvoiceNodeSet/paymentsummary/records[Type='Visa']/AuthorizationAmount)"/>
                    ...
                    <xsl:for-each select="/SalesCredit/result/records[type='CreditMemo']">
                        ...
                        <invoice>
                            ...
                            <tender>
                                ...
                                <Alt3Tender>
                                    <xsl:choose>
                                        <xsl:when test="$CreditMemoNodeSet/TotalAmountWithTax>0">
                                             <xsl:value-of select="translate(concat(format-number($CreditMemoNodeSet/TotalAmountWithTax, '0000000.00'),'-'),'.','')"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                             <xsl:value-of select="translate(concat(format-number($CreditMemoNodeSet/TotalAmountWithTax, '0000000.00'),'-'),'.','')"/>
                            ...
                            <lines>
                                 <xsl:for-each select="$CreditMemolineNodeSet">
                                    <xsl:variable name="Product2Id" select="Product2Id"/>
                                    <xsl:variable name="ProductCode" select="$ProductNodeSet[Id=$Product2Id]/ProductCode"/>
                                     ...
                                     <xsl:choose>
                                         <xsl:when test="Type='Charge'">
                                            <line>
                                                ...
                                                <SpecialOrderVendor/>
                                                <UnitOfMeasure/>
                                                <xsl:variable name="qty" select="'1.0'"/>
                                                 ...
                                                 <xsl:variable name="unitPrice" select="LineAmount"/>
                                                <Quantity>
                                                    <xsl:value-of select="translate(format-number($qty, '00000.000'),'.','')"/>
                                                </Quantity>
                                                <UnitPrice>
